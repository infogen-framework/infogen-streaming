		...
		try {
			...
			if (offset == null) {
				if (auto_offset_reset.equals(AUTO_OFFSET_RESET.smallest.name())) {
					offset = -1l;
				} else if (auto_offset_reset.equals(AUTO_OFFSET_RESET.largest.name())) {
				...
				if (fetchResponse.hasError()) {
					...
					for (MessageAndOffset messageAndOffset : fetchResponse.messageSet(topic, partition)) {
						...
						if (currentOffset < offset) {
							...
							byte[] payload_bytes = new byte[limit];
							payload.get(payload_bytes);

							ByteBuffer key = messageAndOffset.message().key();
							int key_limit = key.limit();
							byte[] key_bytes = new byte[key_limit];
							key.get(key_bytes);

							try {
								mapper.mapper(offset, new String(key_bytes, "UTF-8"), new String(payload_bytes, "UTF-8"), infogen_kafkalzooutputformat);
